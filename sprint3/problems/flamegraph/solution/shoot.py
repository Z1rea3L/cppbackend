...
print(start_server())           # просто для понимания работы argparse
server = run(start_server())    # ./game_server запущен с ./config.json

# perf record
subprocess.Popen(shlex.split("sudo perf record -g -p" + str(server.pid) + " -o perf.data "), stdout=None, stderr=subprocess.DEVNULL)
...
p3.wait() # Wait till process completes
