...
namespace http_handler {
    ...
    public:
        explicit FrontController(application::Application& application, const std::string& static_root, boost::asio::io_context& ioc)
            : application_(application), static_root_(static_root), strand_(boost::asio::make_strand(ioc)) {
        }
        ...
        void operator()(http::request<Body, http::basic_fields<Allocator>>&& req, Send&& send) {
            ...
            if (target.starts_with("/api/")) {
                boost::asio::dispatch(strand_, [this, req = std::move(req), send = std::move(send)]() mutable {
                    api_handler::ApiRequestHandler handlerr(application_);
