...
model::Game LoadGame(const fs::path& config_file) {
    ...
    try {
        ...
        throw std::runtime_error("Wrong config json: defaultDogSpeed");
        }
    ...
    try {
        for (auto& json_map : config.at("maps").as_array()) {
            ...
            try {
                ...
                throw std::runtime_error("Wrong config json: dogSpeed");
                }
...
void LoadRoads(const boost::json::array& roads, model::Map& map){
    for (const auto& road : roads) {
        ...
        try {
            ...
            throw std::runtime_error("Wrong config json: roads");
            }
